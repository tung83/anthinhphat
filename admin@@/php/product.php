...
function product($db){
    ...
    $head_title=array('Tên SP','Đơn Giá(VNĐ)','Danh mục','Hình ảnh','Trang chủ','Hiển thị','Thứ tự');
	$str.=$form->table_start($head_title);
	...
	<div class="row">
        ...
        <div class="col-lg-12">
            ...
            '.$form->ckeditor('content',array('label'=>'Hướng dẫn thuê xe')).'            
            '.$form->ckeditor('manual',array('label'=>'Thủ tục')).'
            '.$form->ckeditor('feature',array('label'=>'Cam kết ATP')).'
...
function product_image($db){
	...
	if(isset($_POST["addNew"])) {
		...
		try{
            ...
            if(common::file_check($_FILES['file'])){
                WideImage::load('file')->resize(800,400, 'fill')->saveToFile(myPath.$file);
                WideImage::load(myPath.$file)->resize(400,200, 'fill')->saveToFile(myPath.'thumb_'.$file);
                $db->where('id',$recent);
	...
	<div class="row">
    	<div class="col-lg-12"><h3>Cập nhật - Thêm mới thông tin</h3></div>
        <div class="col-lg-12">
            '.$form->file('file',array('label'=>'Hình ảnh <code>( 800,400 )</code>')).'
            '.$form->number('ind',array('label'=>'Thứ tự','required'=>true)).'
            '.$form->checkbox('active',array('label'=>'Hiển Thị','checked'=>true)).'
        </div>
    	'.$form->hidden($btn['name'],$btn['value']).'
